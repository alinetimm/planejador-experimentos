...
const COR_RESULTADO = { Passou: "bg-blue-700", Feito: "bg-blue-700", Falhou: "bg-rose-500", Pendente: "bg-amber-500", "N/A": "bg-slate-400" };
...
function Termo({ children, def, ex }) {
  ...
  return (<span className="relative inline-block"><button type="button" onClick={() => setO(!o)} className="text-blue-800 underline decoration-dotted decoration-blue-400 underline-offset-2 cursor-help font-medium">{children}</button>{o && (<span onClick={() => setO(false)} className="absolute z-50 left-0 top-full mt-1 w-56 bg-slate-800 text-white text-xs rounded-lg p-2.5 shadow-lg normal-case text-left leading-snug" style={{ fontWeight: 400 }}>{def}{ex && <span className="block mt-1 text-slate-300"><b>Ex.:</b> {ex}</span>}</span>)}</span>);
}
...
<h2>Referências</h2><ul>${REFERENCIAS.map(r => `<li>${esc(r.t)}</li>`).join("")}</ul><p class="foot">Gerado em ${esc(new Date().toLocaleString("pt-BR"))}</p></body></html>`;
  ...
  if (!modo) {
    return (<div className="min-h-screen bg-slate-50 text-slate-800" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      <header className="bg-gradient-to-r from-slate-900 via-blue-900 to-blue-800 text-white"><div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><img src="/hydrone-mark.png" alt="Hydrone" className="h-9 w-9 shrink-0" /><div><h1 className="text-lg font-bold leading-tight">Planejador de Experimentos</h1><p className="text-xs text-blue-100">Hydrone · guia de metodologia científica</p></div></div>{headerActions}</div></header>
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {Object.entries(FAMILIAS).map(([fk, fam]) => { const Fi = fam.Icon; const modos = Object.entries(MODOS).filter(([, m]) => m.family === fk); return (
          <div key={fk} className="mt-6">
            <div className="flex items-center gap-2 mb-1"><Fi size={18} className="text-blue-700" /><h3 className="text-base font-bold text-slate-800">{fam.label}</h3></div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">{modos.map(([k, m]) => { const Ic = m.Icon; return (
              <button key={k} onClick={() => pickModo(k)} className="text-left bg-white border-2 border-slate-200 hover:border-blue-400 hover:shadow-md transition rounded-xl p-5">
      ...
      <header className="bg-gradient-to-r from-slate-900 via-blue-900 to-blue-800 text-white">
        <div className="max-w-5xl mx-auto px-4 py-4">
          ...
          <nav className="flex gap-1 flex-wrap">{[[1, ClipboardCheck, "Sobre o ensaio"], [2, ClipboardList, STEP2[modo]], [3, BarChart3, STEP3[modo]]].map(([n, Icon, label]) => (
            <button key={n} onClick={() => setStep(n)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition ${step === n ? "bg-white text-blue-800 shadow" : "text-blue-50 hover:bg-blue-600/40"}`}><span className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${step === n ? "bg-blue-700 text-white" : "bg-blue-600/50 text-white"}`}>{n}</span><Icon size={16} /><span className="hidden sm:inline">{label}</span></button>))}
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {step === 1 && (<div className="space-y-6">
          ...
          {modo !== "inspecao" && (<section>
            ...
            <div className="grid grid-cols-3 gap-3">{Object.entries(CONTEXTOS).map(([k, c]) => { const Ic = c.Icon; return (<button key={k} onClick={() => setContexto(k)} className={`rounded-xl border-2 p-4 text-left transition ${contexto === k ? "border-blue-600 bg-blue-50" : "border-slate-200 bg-white hover:border-blue-300"}`}><Ic size={22} className={contexto === k ? "text-blue-700" : "text-slate-500"} /><div className="font-semibold text-sm mt-2">{c.label}</div></button>); })}</div>
            <p className="mt-2 text-sm text-slate-600 flex gap-2 items-start"><Info size={15} className="mt-0.5 text-blue-700 shrink-0" />{CONTEXTOS[contexto].nota}</p>
          </section>)}
          {modo === "comparar" && (<section>
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-2">Objetivo</h2>
            <div className="space-y-2">{OBJETIVOS.map(o => { const sel = objetivo === o.id; return (<button key={o.id} onClick={() => setObjetivo(o.id)} className={`w-full rounded-xl border-2 p-3 text-left transition flex items-start gap-3 ${sel ? "border-blue-600 bg-blue-50" : "border-slate-200 bg-white hover:border-blue-300"}`}><Target size={18} className={`mt-0.5 shrink-0 ${sel ? "text-blue-700" : "text-slate-400"}`} /><div><div className="font-medium text-sm">{o.label}</div><div className="text-xs text-slate-500 italic">“{o.pergunta}”</div></div></button>); })}</div>
            <div className="mt-3 rounded-xl bg-gradient-to-br from-slate-900 via-blue-900 to-blue-800 text-white p-4"><div className="flex items-center gap-2 mb-1"><FlaskConical size={16} /><span className="text-xs uppercase tracking-wide font-semibold text-blue-100">Recomendado</span></div><h3 className="font-bold">{obj.design}</h3><p className="text-sm text-blue-50 mt-1">{obj.interpreta}</p></div>
          </section>)}
          <button onClick={() => setStep(2)} className="w-full bg-blue-700 hover:bg-blue-800 text-white font-semibold py-3 rounded-xl transition">Próximo: {STEP2[modo]} →</button>
        </div>)}
        ...
        {step === 2 && (<div className="space-y-6">
          {modo === "comparar" && (<>
            <section>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-3 text-sm">
                <p className="text-slate-700"><b className="text-blue-900">Fator</b> = aquilo que você muda de propósito para ver o efeito. <b className="text-blue-900">Nível</b> = cada valor que esse fator assume no teste.</p>
                <div className="mt-2 bg-white rounded-md border border-blue-100 text-xs">
                  <div className="flex px-2 py-1.5 border-b border-blue-50 text-slate-500"><span className="w-32">Fator</span><span>Níveis (valores a testar)</span></div>
                  <div className="flex px-2 py-1.5 border-b border-blue-50"><span className="w-32 font-medium text-slate-700">Hélice</span><span className="text-slate-600">A · B · C</span></div>
                  <div className="flex px-2 py-1.5 border-b border-blue-50"><span className="w-32 font-medium text-slate-700">Potência (%)</span><span className="text-slate-600">50 · 75 · 100</span></div>
            ...
            <section>
              ...
              <input type="number" min={1} max={20} value={replicates} onChange={e => setReplicates(Math.max(1, Math.min(20, +e.target.value || 1)))} className="w-24 px-3 py-2 text-sm border border-slate-200 rounded bg-white outline-none focus:border-blue-400" />
              <p className="text-xs text-slate-500 mt-2">Total de ensaios previstos: <span className="font-bold text-blue-800">{nRuns}</span></p>
            ...
            <button onClick={gerarPlano} disabled={!parsedFactors.length} className="w-full bg-blue-700 hover:bg-blue-800 disabled:bg-slate-300 text-white font-semibold py-3 rounded-xl transition">Gerar plano de ensaios (randomizado)</button>
            {matrix.length > 0 && (<section>
              <div className="flex items-center justify-between mb-2"><h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Plano · {matrix.length} ensaios</h2><button onClick={() => download("plano_de_ensaios.csv", toCSV(matrix, matrixCols))} className="text-sm bg-blue-700 hover:bg-blue-800 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5"><Download size={15} /> CSV</button></div>
              <p className="text-xs text-slate-500 mb-2 flex gap-1.5 items-start"><Info size={13} className="mt-0.5 shrink-0 text-blue-700" />A ordem é embaralhada (aleatorização) para que efeitos de tempo não se confundam com os fatores.</p>
          ...
          {modo === "monitorar" && (<>
            <section>
              ...
              <input value={grupoVar} onChange={e => setGrupoVar(e.target.value)} placeholder="ex.: Fase_voo  (deixe vazio se não houver)" className="w-full sm:w-1/2 px-2 py-1.5 text-sm border border-slate-200 rounded outline-none focus:border-blue-400" />
            </section>
            {medidasEditor(medidas, setMedidas, () => setShowLib(true))}
            <p className="text-xs text-slate-500 bg-slate-100 rounded-lg p-3 flex gap-2"><Info size={14} className="mt-0.5 shrink-0 text-blue-700" />Modo observacional: você não controla nada, só registra. A análise descreve o comportamento — não há plano randomizado nem inferência de causa.</p>
          </>)}

          {modo === "inspecao" && (<div className="space-y-4">
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-sm text-slate-700 flex gap-2"><Info size={15} className="mt-0.5 shrink-0 text-blue-700" /><span>Monte o checklist da sessão a partir dos modelos prontos do guia (M1/M7) e/ou adicione seções próprias. Cada sessão carrega só o que for relevante para o ensaio — ex.: voo (M2) = Segurança + Mecânica + Eletrônica + Ambiental.</span></div>
            ...
            <section>
              ...
              <div className="grid sm:grid-cols-2 gap-2">{CHECKLISTS_LIB.map(tpl => { const usado = checklist.some(s => s.tplId === tpl.id); return (
                <button key={tpl.id} onClick={() => toggleSecaoPronta(tpl)} className={`text-left border-2 rounded-lg p-2.5 transition ${usado ? "border-blue-400 bg-blue-50" : "border-slate-200 hover:border-blue-400 hover:bg-blue-50"}`}>
                  <div className="flex items-center justify-between gap-2"><span className="text-sm font-semibold text-slate-800">{tpl.titulo}</span>{usado ? <CheckCircle2 size={15} className="text-blue-700 shrink-0" /> : <Plus size={15} className="text-slate-300 shrink-0" />}</div>
                  <div className="text-xs text-slate-500 mt-0.5">{tpl.fonte} · {tpl.itens.length} itens{usado && <span className="text-blue-700 font-medium"> · adicionado (toque p/ remover)</span>}</div>
                </button>); })}</div>
            </section>

            <section>
              <div className="flex items-center justify-between mb-2"><h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Seções do checklist {checklist.length > 0 && <span className="text-slate-400 normal-case">· {checklistItensTotal} itens</span>}</h2><button onClick={addSecaoVazia} className="text-sm text-blue-700 font-medium flex items-center gap-1"><Plus size={15} /> Seção em branco</button></div>
          ...
          {modo === "inspecao" ? (<section>
            ...
            {checklistItensTotal === 0 ? <p className="text-sm text-slate-500 bg-slate-100 rounded-lg p-4">Monte o checklist na etapa anterior.</p> : (<>
              <div className="space-y-4">{checklist.map(sec => { const c = secCounts(sec); return (<div key={sec.id} className="bg-white border border-slate-200 rounded-xl p-3">
                <div className="flex items-center justify-between gap-2 mb-2 flex-wrap"><h3 className="font-semibold text-sm text-slate-800 flex items-center gap-1.5"><ClipboardList size={15} className="text-blue-700" />{sec.titulo}{sec.fonte && <span className="text-[10px] uppercase tracking-wide text-slate-400 bg-slate-100 rounded px-1.5 py-0.5">{sec.fonte}</span>}</h3><div className="flex gap-1.5 text-xs flex-wrap">{c.status > 0 && <span className="text-slate-500">{c.p}/{c.status} ok</span>}{c.tarefa > 0 && <span className="text-slate-500">{c.status > 0 ? "· " : ""}{c.fe}/{c.tarefa} feito</span>}{c.f > 0 && <span className="text-rose-600 font-medium">· {c.f} falha</span>}{c.pe > 0 && <span className="text-amber-600 font-medium">· {c.pe} pendente</span>}{c.vt > 0 && <span className="text-slate-500">· {c.vp}/{c.vt} registrados</span>}</div></div>
                <div className="space-y-2">{sec.itens.map(it => (<div key={it.id} className="border-b border-slate-100 last:border-0 pb-2 last:pb-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap"><div className="text-sm text-slate-800 flex-1 min-w-40">{it.nome || <span className="text-slate-300">(item sem nome)</span>}</div>
                    {it.captura === "valor" ? (<div className="flex items-center gap-1"><input value={it.valor} onChange={e => setItem(sec.id, it.id, { valor: e.target.value })} placeholder="valor" className="w-28 px-2 py-1 text-sm border border-slate-200 rounded outline-none focus:border-blue-400" />{it.unidade && <span className="text-xs text-slate-400 w-10">{it.unidade}</span>}</div>) : (<div className="flex gap-1.5 items-center">{it.unidade && <input value={it.valor} onChange={e => setItem(sec.id, it.id, { valor: e.target.value })} placeholder={it.unidade} className="w-16 px-1.5 py-1 text-xs border border-slate-200 rounded outline-none focus:border-blue-400" />}{(CAPTURAS[it.captura] || CAPTURAS.status).opcoes.map(r => { const on = it.resultado === r; return (<button key={r} onClick={() => setItem(sec.id, it.id, { resultado: on ? "" : r })} className={`text-xs px-2.5 py-1 rounded-lg font-medium ${on ? (COR_RESULTADO[r] || "bg-slate-400") + " text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{r}</button>); })}</div>)}
                  </div>
                  <input value={it.obs} onChange={e => setItem(sec.id, it.id, { obs: e.target.value })} placeholder="observação / link de evidência (foto, log...)" className="w-full mt-1.5 px-2 py-1 text-xs border border-slate-200 rounded outline-none focus:border-blue-400" />
                </div>))}</div>
              </div>); })}</div>
              <div className="mt-3 flex flex-wrap gap-2 text-sm">{checkCounts.statusTotal > 0 && <><span className="bg-blue-50 text-blue-800 rounded-lg px-3 py-1.5 font-medium">{checkCounts.Passou} passou</span><span className="bg-rose-50 text-rose-700 rounded-lg px-3 py-1.5 font-medium">{checkCounts.Falhou} falhou</span></>}{checkCounts.tarefaTotal > 0 && <><span className="bg-blue-50 text-blue-800 rounded-lg px-3 py-1.5 font-medium">{checkCounts.Feito} feito</span><span className="bg-amber-50 text-amber-700 rounded-lg px-3 py-1.5 font-medium">{checkCounts.Pendente} pendente</span></>}{checkCounts["N/A"] > 0 && <span className="bg-slate-100 text-slate-600 rounded-lg px-3 py-1.5 font-medium">{checkCounts["N/A"]} N/A</span>}{checkCounts.valorTotal > 0 && <span className="bg-blue-50 text-blue-800 rounded-lg px-3 py-1.5 font-medium">{checkCounts.valorPreenchidos}/{checkCounts.valorTotal} valores</span>}</div>
            </>)}
          </section>) : (<>
            <section>
              <div className="inline-flex rounded-lg border border-slate-200 bg-white p-0.5 mb-3"><button onClick={() => setEntryMode("inserir")} className={`text-sm px-3 py-1.5 rounded-md flex items-center gap-1.5 ${entryMode === "inserir" ? "bg-blue-700 text-white" : "text-slate-600"}`}><Pencil size={14} /> Inserir no app</button><button onClick={() => setEntryMode("importar")} className={`text-sm px-3 py-1.5 rounded-md flex items-center gap-1.5 ${entryMode === "importar" ? "bg-blue-700 text-white" : "text-slate-600"}`}><Upload size={14} /> Importar CSV</button></div>
              {entryMode === "inserir" && (<div>
                <div className="flex flex-wrap gap-2 mb-2"><button onClick={loadPlanIntoGrid} className="text-sm bg-blue-700 hover:bg-blue-800 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5"><Table2 size={15} /> {modo === "comparar" && matrix.length ? "Preencher do plano" : "Criar tabela"}</button>{rows.length > 0 && <button onClick={addRow} className="text-sm bg-white border border-slate-200 text-slate-700 px-3 py-1.5 rounded-lg flex items-center gap-1.5"><Plus size={15} /> Adicionar ensaio</button>}{rows.length > 0 && <button onClick={() => download("dados_ensaios.csv", toCSV(rows, headers))} className="text-sm bg-white border border-slate-200 text-slate-700 px-3 py-1.5 rounded-lg flex items-center gap-1.5"><Download size={15} /> Baixar</button>}</div>
              ...
              {entryMode === "importar" && (<div><div className="flex flex-wrap gap-3 items-center"><label className="cursor-pointer bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-4 py-2 rounded-lg flex items-center gap-2"><Upload size={15} /> Carregar CSV<input type="file" accept=".csv" onChange={handleFile} className="hidden" /></label>{modo === "comparar" && <button onClick={() => ingest(EXEMPLO)} className="text-sm bg-slate-200 hover:bg-slate-300 text-slate-700 px-4 py-2 rounded-lg font-medium">Carregar exemplo</button>}</div><details className="mt-3"><summary className="text-sm text-blue-700 cursor-pointer">ou colar (CSV)</summary><textarea onChange={e => e.target.value.trim() && handlePaste(e.target.value)} placeholder={"Config,Empuxo_N\nHélice A,12.1"} className="mt-2 w-full h-24 p-2 text-sm font-mono border border-slate-200 rounded outline-none focus:border-blue-400" /></details>{loadErr && <p className="text-sm text-rose-600 mt-2">{loadErr}</p>}</div>)}
            </section>
            ...
            {rows.length > 0 && (modo === "comparar" || modo === "monitorar") && (<>
              <section className="grid sm:grid-cols-2 gap-4 bg-white rounded-xl border border-slate-200 p-4">
                <div><label className="text-xs font-semibold text-slate-500 uppercase">{modo === "monitorar" ? "Agrupar por" : "Fator (grupo)"}</label><select value={groupCol} onChange={e => setGroupCol(e.target.value)} className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded bg-white outline-none focus:border-blue-400">{headers.map(h => <option key={h}>{h}</option>)}</select></div>
                <div><label className="text-xs font-semibold text-slate-500 uppercase">Variável medida</label><select value={respCol} onChange={e => setRespCol(e.target.value)} className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded bg-white outline-none focus:border-blue-400">{respSelectList.map(h => <option key={h}>{h}</option>)}</select><p className="text-xs text-slate-400 mt-1">Tipo: <b className="text-slate-600">{TIPOS[tipoResp].label}</b> → {TIPOS[tipoResp].analise}.</p></div>
              </section>
              ...
              {analysis?.kind === "categorica" && (<>
                ...
                <section><h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-2">Teste <Termo def={TERMOS.qui.def} ex={TERMOS.qui.ex}>qui-quadrado</Termo></h2><p className="text-sm">X² = <b>{analysis.ct.X2.toFixed(3)}</b> · gl = {analysis.ct.df} · p = <b className={analysis.ct.p < 0.05 ? "text-blue-800" : "text-slate-500"}>{analysis.ct.p < 0.0001 ? "<0,0001" : analysis.ct.p.toFixed(4)}</b></p><p className={`mt-2 text-sm rounded-lg px-3 py-2 ${analysis.ct.p < 0.05 ? "bg-blue-50 text-blue-900" : "bg-amber-50 text-amber-800"}`}>{analysis.ct.p < 0.05 ? (modo === "monitorar" ? "Associação significativa entre grupo e categoria (p < 0,05)." : "O resultado depende do grupo (p < 0,05).") : "Sem evidência de associação ao nível de 5%."}</p></section>
              </>)}
            </>)}
          </>)}
        </div>)}
